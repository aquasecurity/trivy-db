...
func (vs VulnSrc) commit(tx *bolt.Tx, ecosystem types.Ecosystem, entries []Entry) error {
	...
	for _, entry := range entries {
		...
		if ecosystem == vulnerability.Swift {
			...
			// Add them to fit the same format
			if !strings.HasPrefix(entry.Package.Name, "https://") {
				entry.Package.Name = fmt.Sprintf("https://%s", entry.Package.Name)
			}
			if !strings.HasSuffix(entry.Package.Name, ".git") {
				entry.Package.Name = fmt.Sprintf("%s.git", entry.Package.Name)
			}
		}
		var pvs, avs []string
